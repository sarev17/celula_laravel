<?php
    ...
    class Product{
        ...
        public function create(){
            $query = "INSERT INTO products (name, price, stock, category,time_delivery,image)
            VALUES ('$this->name','$this->price', '$this->stock', '$this->category','$this->time_delivery','$this->image');";
            return $this->response($query,'Produto cadastrado.','Erro ao cadastrar produto',201,400);
        ...
        function update(){
            ...
            if(!empty($this->purchases)){
                $set[] = "purchases = '{$this->purchases}'";
            }
            if(!empty($this->time_delivery)){
                $set[] = "time_delivery = '{$this->time_delivery}'";
            }
            if(!empty($this->image)){
